...
void IO_init(void)
{
	...
	port_pin_set_output_level(PIN_PA00, false);	//	Y4
	port_pin_set_config(PIN_PA01, &pin_conf);
	port_pin_set_output_level(PIN_PA01, false);	//	Y3
	port_pin_set_config(PIN_PA04, &pin_conf);
	port_pin_set_output_level(PIN_PA04, false);	//	Y2
	port_pin_set_config(PIN_PA05, &pin_conf);
	port_pin_set_output_level(PIN_PA05, false);	//	Y1
	
	port_pin_set_config(PIN_PA07, &pin_conf);
	port_pin_set_output_level(PIN_PA07, true);	//	LoRA POWER ENABLE
	port_pin_set_config(PIN_PA22, &pin_conf);
	port_pin_set_output_level(PIN_PA22, true);	//	HX711 LDT
	port_pin_set_config(PIN_PA16, &pin_conf);
	port_pin_set_output_level(PIN_PA16, true);	//	HX711 SCK
	
	
	//	RGB LED PIN DEFINE
	port_pin_set_config(PIN_PA23, &pin_conf);
	port_pin_set_output_level(PIN_PA23, true);	//	LED GREEN
	port_pin_set_config(PIN_PA27, &pin_conf);
	port_pin_set_output_level(PIN_PA27, true);	//	LED RED
	port_pin_set_config(PIN_PA28, &pin_conf);
	port_pin_set_output_level(PIN_PA28, true);	//	LED BLUE

	
	pin_conf.direction  = PORT_PIN_DIR_INPUT;
	port_pin_set_config(PIN_PA11, &pin_conf);
	port_pin_set_output_level(PIN_PA11, false);	//	LoRA IRQ
	port_pin_set_config(PIN_PA17, &pin_conf);
	port_pin_set_output_level(PIN_PA17, false);	//	NA

	port_pin_set_config(PIN_PA02, &pin_conf);
	port_pin_set_output_level(PIN_PA02, true);	//	Hall Sensor 1
	port_pin_set_config(PIN_PA03, &pin_conf);
	port_pin_set_output_level(PIN_PA03, true);	//	Hall Sensor 2
	
	port_pin_set_config(PIN_PA24, &pin_conf);
	port_pin_set_output_level(PIN_PA24, true);	//	Din 2
	port_pin_set_config(PIN_PA25, &pin_conf);	//	Din 1
...
void Dot_Display(void)
{
	...
	uTXByte(Display_COM, seg7_dot + 0x30);
	uTXByte(Display_COM, '0');
	uTXByte(Display_COM, '0');
	uTXByte(Display_COM, '0');
	uTXByte(Display_COM, '0');
